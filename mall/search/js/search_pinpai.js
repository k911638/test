$(function() {
	...
	function qingqiu(pages, goods) {
		...
		$.ajax({
			...
			.done(function(data) {
				...
				<div> 
					<h3>${data.data[i].brandName} ${data.data[i].name}</h3>
		...
		//请求品牌分类
		$.ajax({
				url: geturl + 'brand/brand/searchBrand.do',
				type: 'post',
				dataType: 'json',
				contentType: 'application/json;charset=utf-8',
				data: JSON.stringify({
			...
			.done(function({
				...
				for (var i = 0; i < data.length; i++) {
					...
					var $li = $('<li><img src="' + data[i].logo + '" alt="其它商品"><p>' + data[i].name + '</p></li>');
					$li.appendTo($fenlei);
				}
			})
			.fail(function() {
	...
	//点击商品分类
	const $fenlei = $(".fenlei");
	$fenlei.delegate('li', 'click', function() {
		var goods = $(this).children().eq(1).text();
		pages = 1;
		qingqiu(pages, goods);
	});
	...
	const $center = $(".center");
	var timer = null;
	var $fenleicon = $(".fenlei_con");
	$center.scroll(function() {
		clearTimeout(timer);
		timer = setTimeout(function() {
			const results = $center.scrollTop() + $center.height() - $list.height() - $fenleicon.height();;
			//console.log(results);
