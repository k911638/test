...
$(function() {
	...
	// 点击搜索
	const search_text = $(".search_con input");
	$(".search_btn").click(function() {
		...
		qingqiu(pages);
		//历史跟新去重
		if (history_arr.indexOf(val) != -1) {
			return;
		}
		history_arr.length > 10 ? history_arr.pop() : null;
		history_arr.unshift(val);
		history_str = history_arr.join(",");
		localStorage.setItem("history_str", history_str);
		get_history();
	});
	//搜索框与历史框
	search_text.focus(function() {
		$history.show().next().hide();
	});

	//历史
	const $history = $('.history');
	const history2 = $(".history_2");
	var history_arr = [];
	var history_str = localStorage.getItem("history_str");
	...
	function get_history() {
		...
		for (let i = 0; i < history_arr.length; i++) {
			let $li = $("<li>" + history_arr[i] + "</li>");
			$li.appendTo(history2);
		}
	}
	$(".history_1 img").click(function() { //清空历史
		history2.empty();
		localStorage.setItem("history_str", "");
		history_arr = [];
	});
	//点击历史项
	$history.delegate('li', 'click', function() {
		val = $(this).text();
		search_text.val(val);
		$history.hide().next().show();
		pages = 1;
